...
#define DRAG_DROP_PASTEBOARD
    ...
    textData = CFDataCreate( kCFAllocatorDefault,  (UInt8*)buffer, sizeof( UniChar ) * numUniChars );
...
OSStatus ScintillaMacOSX::DragReceive(DragRef inDrag )
{
    ...
    if ( pdoc->InsertString( position, cstring + bomLen, usedBufferLength - bomLen ) )
    {
        SetEmptySelection( currentPos + usedBufferLength - bomLen );
    }
...
void ScintillaMacOSX::SyncPaint(void* gc, PRectangle rc) {
    ...
    if (paintState == paintAbandoned) {
        ...
        //FullPaint( gc );
        // On Mac OS X, Quartz clips drawing to the invalid area anyway. So we invalidate
        // the entire area, and then we will get called again to paint everything.
...
void ScintillaMacOSX::Resize(int width, int height) {
    ...
    if (HIViewGetSuperview(GetViewRef()) != NULL) {
        ...
        PRectangle r = GetClientRectangle();
...
bool ScintillaMacOSX::CanPaste()
{
    ...
    err = GetScrapFlavorFlags ( scrap, kScrapFlavorTypeUnicode, &flavorFlags );

    if ( err == noErr ) {
        return true;
    }
    
    assert( err == scrapFlavorNotFoundErr );
    return false;
}
...
void ScintillaMacOSX::Paste(bool isRectangular)
{
    ...
    if (isRectangular) {
        ...
        PasteRectangular(selStart, cstring + bomLen, usedBufferLength - bomLen);
    } else 
    if ( pdoc->InsertString( currentPos, cstring + bomLen, usedBufferLength - bomLen ) ) {
        SetEmptySelection( currentPos + usedBufferLength - bomLen );
    }
